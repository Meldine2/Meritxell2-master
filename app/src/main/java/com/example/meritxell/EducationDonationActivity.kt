...
class EducationDonationActivity : AppCompatActivity() {
    ...
    private lateinit var educationTypeEditText: EditText
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        setContentView(R.layout.activity_education_donation)
        ...
        educationTypeEditText = findViewById(R.id.educationTypeEditText)
    ...
    private fun submitForm() {
        ...
        val educationType = educationTypeEditText.text.toString().trim()

        if (!validateInputs(fullName, email, phone, address1, city, state, zip, educationType)) {
            return
        }
        ...
        val userData = hashMapOf(
            ...
            "state" to state,
            "zip" to zip,
            "educationType" to educationType,
            "timestamp" to FieldValue.serverTimestamp(),
            "status" to "pending", // Default status for admin review
            "userId" to (currentUser?.uid ?: ""), // Track which user submitted
            "donationType" to "education"
        )

        firestore.collection("educationdonation")
            .document(docId)
            .set(userData)
            .addOnSuccessListener {
                Log.d("EducationDonation", "Education donation submitted successfully with ID: $docId")
                
                // Create automatic chat connection with admin
                createDonationChatConnection(docId, fullName, "education")
                
                Toast.makeText(this, "Education donation submitted successfully! Admin will contact you shortly.", Toast.LENGTH_LONG).show()
                clearForm()
            ...
            .addOnFailureListener { e ->
                Log.e("EducationDonation", "Failed to submit education donation: ${e.message}")
    ...
    private fun createDonationChatConnection(donationId: String, donorName: String, donationType: String) {
        ...
        if (userId.isNullOrEmpty()) {
            Log.w("EducationDonation", "User not authenticated, cannot create chat connection")
            return
        }
        ...
        firestore.collection("users").document(userId).get()
            .addOnSuccessListener { userDoc ->
                ...
                Log.d("EducationDonation", "Chat connection created for $donationType donation: $donationId")
            }
            .addOnFailureListener { e ->
                Log.e("EducationDonation", "Failed to get user data for chat connection: ${e.message}")
    ...
    private fun validateInputs(
        ...
        if (educationType.length < 3) {
            educationTypeEditText.error = "Please enter at least 3 characters describing the education type"
            educationTypeEditText.requestFocus()
            return false
        }
    ...
    private fun clearForm() {
        ...
        educationTypeEditText.text.clear()
    }
}
