...
class FoodDonationActivity : AppCompatActivity() {
    ...
    private fun submitForm() {
        ...
        if (!validateInputs(fullName, email, phone, address1, city, state, zip, foodType)) {
            return
        }
        ...
        val userData = hashMapOf(
            ...
            "donationType" to "food"
        )

        firestore.collection("fooddonation")
            .document(docId)
            .set(userData)
            .addOnSuccessListener {
                Log.d("FoodDonation", "Food donation submitted successfully with ID: $docId")
                
                // Create automatic chat connection with admin
                createDonationChatConnection(docId, fullName, "food")
                
                Toast.makeText(this, "Food donation submitted successfully! Admin will contact you shortly.", Toast.LENGTH_LONG).show()
                clearForm()
            ...
            .addOnFailureListener { e ->
                Log.e("FoodDonation", "Failed to submit food donation: ${e.message}")
    ...
    private fun createDonationChatConnection(donationId: String, donorName: String, donationType: String) {
        ...
        if (userId.isNullOrEmpty()) {
            Log.w("FoodDonation", "User not authenticated, cannot create chat connection")
            return
        }
        ...
        firestore.collection("users").document(userId).get()
            .addOnSuccessListener { userDoc ->
                ...
                Log.d("FoodDonation", "Chat connection created for $donationType donation: $donationId")
            }
            .addOnFailureListener { e ->
                Log.e("FoodDonation", "Failed to get user data for chat connection: ${e.message}")
    ...
    private fun validateInputs(
        ...
        if (foodType.length < 3) {
            foodTypeEditText.error = "Please enter at least 3 characters describing the food type"
