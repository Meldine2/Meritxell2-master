...
class ClothesDonationActivity : AppCompatActivity() {
    ...
    private lateinit var clothesTypeEditText: EditText
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        clothesTypeEditText = findViewById(R.id.clothesTypeEditText)
    ...
    private fun submitForm() {
        ...
        val clothesType = clothesTypeEditText.text.toString().trim()

        if (!validateInputs(fullName, email, phone, address1, city, state, zip, clothesType)) {
            return
        }
        ...
        val userData = hashMapOf(
            ...
            "state" to state,
            "zip" to zip,
            "clothesType" to clothesType,
            "timestamp" to FieldValue.serverTimestamp(),
            "status" to "pending", // Default status for admin review
            "userId" to (currentUser?.uid ?: ""), // Track which user submitted
            "donationType" to "clothes"
        )

        firestore.collection("clothesdonation")
            .document(docId)
            .set(userData)
            .addOnSuccessListener {
                Log.d("ClothesDonation", "Clothes donation submitted successfully with ID: $docId")
                
                // Create automatic chat connection with admin
                createDonationChatConnection(docId, fullName, "clothes")
                
                Toast.makeText(this, "Clothes donation submitted successfully! Admin will contact you shortly.", Toast.LENGTH_LONG).show()
                clearForm()
            ...
            .addOnFailureListener { e ->
                Log.e("ClothesDonation", "Failed to submit clothes donation: ${e.message}")
    ...
    private fun createDonationChatConnection(donationId: String, donorName: String, donationType: String) {
        ...
        if (userId.isNullOrEmpty()) {
            Log.w("ClothesDonation", "User not authenticated, cannot create chat connection")
            return
        }
        ...
        firestore.collection("users").document(userId).get()
            .addOnSuccessListener { userDoc ->
                ...
                Log.d("ClothesDonation", "Chat connection created for $donationType donation: $donationId")
            }
            .addOnFailureListener { e ->
                Log.e("ClothesDonation", "Failed to get user data for chat connection: ${e.message}")
    ...
    private fun validateInputs(
        ...
        if (clothesType.length < 3) {
            clothesTypeEditText.error = "Please enter at least 3 characters describing the clothes type"
            clothesTypeEditText.requestFocus()
            return false
        }
    ...
    private fun clearForm() {
        ...
        clothesTypeEditText.text.clear()
    }
}
